...
function crearUsuario($conn, $nombre, $apellido, $nucuenta, $numtelefono, $email, $pwd){
	...
	mysqli_stmt_bind_param($stmt, "ssssss", $nombre, $apellido, $nucuenta, $numtelefono, $email, $pwd);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_close($stmt);
...
function loginUser($conn, $email, $nucuenta, $pwd){
		...
		session_start();
		$query = "SELECT ID, nombre, apellido, num_cuenta, num_telefono, correo, contrasenia, foto, banner, imgType, bio FROM usuarios WHERE correo = '$email' and contrasenia = '$pwd' ";
